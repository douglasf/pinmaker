 ...
 * @param {import('./types.js').PinConfig} config
 * @returns {import('./types.js').CirclePosition[]}
 */
export function calculateLayout(totalCircles, config) {
  const positions = [];
 ...
 * @returns {{ rows: number; cols: number; startX: number; startY: number }}
 */
function calculatePageLayout(circleCount, circleDiameter) {
 ...
 * @param {import('./types.js').CirclePosition[]} positions
 * @returns {number}
 */
export function getTotalPages(positions) {
  if (positions.length === 0) return 0;
  return Math.max(...positions.map(p => p.page)) + 1;
}

/**
 * Create image distribution array for duplication
 * @param {number} imageCount
 * @param {number} totalCircles
 * @returns {number[]}
 */
export function createImageDistribution(imageCount, totalCircles) {
  const distribution = [];
